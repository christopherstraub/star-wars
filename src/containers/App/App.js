...
class App extends Component {
  ...
  render() {
    ...
    // We want to filter out the date edited, date created,
    // and people so that the user can't search for their values.

    // If homeworld is equal to null we set it equal to "no homeworld"
    // (as occurs the "Droid" species object).

    // We append units where necessary, so that a user search including units
    // will still display an object whose original data does not include the units.
    const filteredResourceValues = resourceData.map(
      (array, i, arrayResources) => {
        return array.map((object) => {
          const { edited, created, people, ...rest } = object;
          ...
          if (typeof rest.homeworld === 'number')
            rest.homeworld = arrayResources[1][rest.homeworld - 1].name;
          else {
            if (rest.homeworld === null) {
              object.homeworld = 'no homeworld';
            }
          }
          if (
